...
import { WSMessage, FileSyncEvent } from '@/types';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    newSocket.on('message', (message: WSMessage) => {
      ...
      if (message.type === 'file_sync_start' ||
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600 mb-6">
          Monitor and sync files to Telegram channels
        </p>
              ...
              <div key={`${event.id}-${index}`} className="border-l-4 border-blue-500 pl-4 py-2">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <span className={`inline-block px-2 py-1 text-xs font-medium rounded ${event.status === 'completed' ? 'bg-green-100 text-green-800' :
                      event.status === 'error' ? 'bg-red-100 text-red-800' :
                        event.status === 'syncing' ? 'bg-yellow-100 text-yellow-800' :
                ...
                {event.error && (
                  <p className="text-sm text-red-600 mt-2">{event.error}</p>
                )}
              </div>
            ))}
          </div>
